...
module Paperclip
  ...
  class Attachment
    ...
    #SARAV-REPROCESS-BEGIN
    def oldpath style_name = default_style
      original_filename.nil? ? nil : interpolate(@oldpath, style_name)
    end
    #SARAV-REPROCESS-END
    ...
    def reprocesspaths!
      ...
      unless @oldpath.blank?
        old_original = to_file_old(:original)
        self.clearold
      else
      ...
      if old_original
        ...
        unless @oldpath.blank?
          instance_write(:fingerprint,     generate_fingerprint(new_original))
          instance_write(:updated_at,      Time.now)
        end
    ...
    #SARAV-REPROCESS-BEGIN
    ###
    # This function queues the files with old path for deletion. Since only the
    # path of the file will change, we should not touch the other attributes
    # of the instance like file_name, size etc
    ###
    def queue_oldexisting_for_delete
      @queued_for_delete = []
      @queued_for_delete << oldpath(:original)
      styles.each do |name, style|
        @queued_for_delete << oldpath(name)
      end
      #@queued_for_delete += [:original, *styles.keys].uniq.map do |style|
      #  oldpath(style) if exists?(style)
      #end.compact
    end
    #SARAV-REPROCESS-END
